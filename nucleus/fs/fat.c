...
void fs_check_fat (BPB_FAT* fat)
{
    ...
    TOS_CHECK (fat->bpb_byts_per_sec == 512 ||
	       fat->bpb_byts_per_sec == 1024 ||
	       fat->bpb_byts_per_sec == 2048 ||
	       fat->bpb_byts_per_sec == 4096);
...
void fs_init (TOS_FAT_Device* dev)
{
    ...
    dev->bytes_per_sector = fat->bpb_byts_per_sec;
    
    // Size of the FAT in number of sectors
    if (fat->bpb_fat_sz16 != 0)
	dev->fat_size = fat->bpb_fat_sz16;
    else
	dev->fat_size = fat->u.fat32.bpb_fat_sz32;
    
    dev->sectors_per_cluster = fat->bpb_sec_per_clus;

    dev->rsvd_sec_cnt = fat->bpb_rsvd_sec_cnt;
    
    // Compute number of sectors for root directory
    dev->root_dir_secs = ((fat->bpb_root_ent_cnt * 32) +
			  (fat->bpb_byts_per_sec - 1)) /
	fat->bpb_byts_per_sec;

    // Compute first root dir sector
    dev->first_root_dir_sector = fat->bpb_rsvd_sec_cnt +
	(fat->bpb_num_fats * dev->fat_size);
    
    // Compute first data sector
    dev->first_data_sector = fat->bpb_rsvd_sec_cnt +
	(fat->bpb_num_fats * dev->fat_size) + dev->root_dir_secs;

    // # of sectors in the volume
    if (fat->bpb_tot_sec16 != 0)
	dev->total_sectors = fat->bpb_tot_sec16;
    else
	dev->total_sectors = fat->bpb_tot_sec32;
    
    // Compute the # of sectors in the data region of the volume
    dev->total_data_sectors = dev->total_sectors - 
	(fat->bpb_rsvd_sec_cnt + (fat->bpb_num_fats * dev->fat_size) +
	 dev->root_dir_secs);
...
TOS_UInt32 fs_get_fat_cluster_entry (TOS_FAT_Device* dev, TOS_UInt32 cluster)
{
  ...
  if (dev->fat_type == FAT12_TYPE)
    {
      value = *((TOS_UInt16*) &dev->fat[offset]);
    ...
    {
      value = *((TOS_UInt32*) &dev->fat[offset]);
    }
...
void fs_set_fat_cluster_entry (TOS_FAT_Device* dev, TOS_UInt32 cluster, TOS_UInt32 value)
{
  ...
  if (dev->fat_type == FAT12_TYPE)
	...
	{
	  value = value << 4;
	  *((TOS_UInt16*) &dev->fat[offset]) = (*((TOS_UInt16*) &dev->fat[offset])) & 0x000F;
	}
      else
	{
	  value = value & 0x0FFF;
	  *((TOS_UInt16*) &dev->fat[offset]) = (*((TOS_UInt16*) &dev->fat[offset])) & 0xF000;
	}

      *((TOS_UInt16*) &dev->fat[offset]) = (*((TOS_UInt16*) &dev->fat[offset])) | value;
    }
  else if (dev->fat_type == FAT16_TYPE)
    {
      *((TOS_UInt16*) &dev->fat[offset]) = value;
    }
  else if (dev->fat_type == FAT32_TYPE)
    {
      value = value & 0x0FFFFFFF;
      *((TOS_UInt32*) &dev->fat[offset]) = *((TOS_UInt32*) &dev->fat[offset]) & 0xF0000000;
      *((TOS_UInt32*) &dev->fat[offset]) = *((TOS_UInt32*) &dev->fat[offset]) | value;
    }

  // set FAT cluster entry on device

  if ((dev->fat_type == FAT12_TYPE) && (thisFATEntOffset == (dev->bytes_per_sector - 1)))
    fs_bio_write(&dev->fat[(offset / dev->bytes_per_sector) * dev->bytes_per_sector], thisFATSecNum, 2);
  else
    fs_bio_write(&dev->fat[(offset / dev->bytes_per_sector) * dev->bytes_per_sector], thisFATSecNum, 1);
}
		      ...
		      if (dir_entries[k].dir_name[0] != 0xE5)
			{
			  ...
			  if (strcmp(current_file_name, file_name) == 0)
				...
				continue;
			      ...
			      current_cluster = dir_entries[k].dir_fst_clus_hi;
			      current_cluster = (current_cluster << 8) + dir_entries[k].dir_fst_clus_lo;
...
TOS_UInt32 fs_create_directory(TOS_FAT_Device* dev, TOS_Octet* parent_directory, TOS_Octet* child_directory)
{
  ...
  subdir_entry.dir_fst_clus_lo = cluster;
  subdir_entry.dir_fst_clus_hi = cluster >> 16; // always 0 for a FAT12 or FAT16 volume
  subdir_entry.dir_attr = 0x10;
  subdir_entry.dir_nt_res = 0x00;
  subdir_entry.dir_crt_time_tenth = 0x00;
  subdir_entry.dir_crt_time = 0x00;
  subdir_entry.dir_crt_date = 0x00;
  subdir_entry.dir_lst_acc_date = 0x00;
  subdir_entry.dir_wrt_time = 0x00;
  subdir_entry.dir_wrt_date = 0x00;
  subdir_entry.dir_file_size = 0x00;
  ...
  dir_entries[0].dir_fst_clus_lo = cluster;
  dir_entries[1].dir_fst_clus_lo = parent_cluster;

  // always 0 for a FAT12 or FAT16 volume
  dir_entries[0].dir_fst_clus_hi = cluster >> 16;
  dir_entries[1].dir_fst_clus_hi = parent_cluster >> 16;
  ...
  for (i = 0; i < 2; i++)
    {
      ...
      dir_entries[i].dir_crt_time = 0x00;
      dir_entries[i].dir_crt_date = 0x00;
      dir_entries[i].dir_lst_acc_date = 0x00;
      dir_entries[i].dir_wrt_time = 0x00;
      dir_entries[i].dir_wrt_date = 0x00;
      dir_entries[i].dir_file_size = 0x00;
    }
...
TOS_fs_next_cluster_in_chain (TOS_FAT_Device* dev, TOS_UInt32 cluster)
{
	...
	next_cluster = *((TOS_UInt16*) &(dev->fat [fat_offset]));
	if (cluster & 1)
	...
	next_cluster = *((TOS_UInt32*) &(dev->fat [fat_offset]));
	break;
    }
...
TOS_Error fs_open (TOS_FAT_Device* dev, TOS_Octet* path, TOS_FS_OPEN_MODE mode)
{
  ...
  if (mode == TOS_FS_OPEN_MODE_READ)
	...
	return -1;
      ...
      fcb[fcb_index].file_size = dir_entry.dir_file_size;
      fcb[fcb_index].dev = dev;
      fcb[fcb_index].pos = 0;
      fcb[fcb_index].first_cluster = dir_entry.dir_fst_clus_lo | (dir_entry.dir_fst_clus_hi << 16);
	...
	if (fs_get_directory_entry (dev, path, &dir_entry))
	...
	else
	  {
	    ...
	    dir_entry.dir_fst_clus_lo = cluster;
	    dir_entry.dir_fst_clus_hi = cluster >> 16; // always 0 for a FAT12 or FAT16 volume
	    dir_entry.dir_attr = 0x00;
	    dir_entry.dir_nt_res = 0x00;
	    dir_entry.dir_crt_time_tenth = 0x00;
	    dir_entry.dir_crt_time = 0x00;
	    dir_entry.dir_crt_date = 0x00;
	    dir_entry.dir_lst_acc_date = 0x00;
	    dir_entry.dir_wrt_time = 0x00;
	    dir_entry.dir_wrt_date = 0x00;
	    dir_entry.dir_file_size = 0x00;
  ...
  if (fcb[i].mode == TOS_FS_OPEN_MODE_WRITE) // flush
    {
      ...
      dir_entry.dir_file_size = fcb[i].file_size;
