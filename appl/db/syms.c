...
static symtree_print(word32 s, int byval, int level)
{
  ...
  wl_printf( debug_wid, "0x%08lx 0x%08lx %*s%s\n\r", tmp.val, tmp.type, level, "", tmps);
...
void syms_init(char *fname)
{
  ...
  FILENODE filen;
  LINENODE linen;
  int handles[] = { DEFAULT_FILE, DEFAULT_FILE, DEFAULT_FILE };
  static init_flag = 0;

  if( init_flag == 1 ) {
  	wl_printf( debug_wid, "Program already loaded!\n\r" );
  	return;
  }
  filen.me = 0;
  linen.me = 0;
  ...
  sfd = fopen(fname, "rb");
  if( sfd == NULL ) {
  	wl_printf( debug_wid, "Can't open file\n\r" );
  	return;
  }
  init_flag = 1;

  /*
   * Get program to be debugged running.
   */
  hermes_exec_debug_flag = TRUE;
  hermes_exec( fname, NULL, environ, handles );
  hermes_exec_debug_flag = FALSE;
  
  wl_printf( debug_wid, "Loading %s.\n\r", fname );
  ...
  wl_printf( debug_wid, "Reading symbols...   0%%");
  ...
  for (i=0; i<nsyms; i++)
  {
    ...
    if (per != oldper)
    {
      wl_printf( debug_wid, "\b\b\b\b%3d%%", per);
      oldper = per;
    }
  ...
  wl_printf( debug_wid, " %ld symbol%s read\n\r", nsyms, nsyms==1 ? "" : "s");
  return;
}
...
*/
    ...
    if (name[i] == '#')
    {
      ...
      for (f.me=file_list; f.me; f.me=f.next)
      {
        ...
        if ((strncmp(name, tmps, i) == 0) && (tmps[i] == 0))
        {
          ...
          wl_printf( debug_wid, "undefined line number %.*s\n\r", idx, name);
          undefined_symbol = 1;
          return 0;
        }
      }
      wl_printf( debug_wid, "Undefined file name %.*s\n\r", i, name);
      undefined_symbol = 1;
      return 0;
    }
  ...
  wl_printf( debug_wid, "Undefined symbol %.*s\n\r", idx, name);
  undefined_symbol = 1;
  return 0;
}
...
#endif
      wl_printf( debug_wid, "\r            \r");
    }
    wl_printf( debug_wid, "0x%08lx %c %s", s.val, type2char(s.type), tmps);
    name = syms_val2line(s.val, &lnum, 0);
    if (name)
      wl_printf( debug_wid, ", line %d of %s", lnum, name);
#if 0
    mputchar('\n\r');
#endif
	wl_printf( debug_wid,  "\n\r\r" );
